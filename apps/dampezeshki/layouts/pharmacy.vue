        ...
        <div class="p-6 border-b border-gray-200">
          <div class="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">{{ currentPharmacyName }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 border-t border-gray-200">
          <button @click="showProfileMenu = !showProfileMenu" class="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
            ...
            <div class="flex-1 text-right">
              ...
              <p class="text-xs text-gray-500">admin@pharmacy.com</p>
            </div>
          ...
          <Transition name="slide-up">
            <div v-if="showProfileMenu" class="mt-2 p-2 bg-gray-50 rounded-lg space-y-1">
              <NuxtLink to="/panel/pharmacies/settings" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-white rounded-lg transition-colors">
      ...
      <aside v-if="mobileSidebarOpen" class="fixed top-0 right-0 w-64 h-screen bg-white z-[60] lg:hidden overflow-y-auto">
        <div class="flex flex-col h-full">
          ...
          <div class="p-4 border-t border-gray-200">
            ...
            <div class="space-y-1">
              <NuxtLink to="/panel/pharmacies/settings" @click="toggleMobileSidebar" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                <i class="ti ti-settings text-base"></i>
                تنظیمات
              </NuxtLink>
      ...
      <header class="hidden lg:block sticky top-0 bg-white border-b border-gray-200 z-30">
        <div class="flex items-center justify-between px-6 py-4">
          <div class="flex items-center gap-4">
            <button
              v-if="route.path !== '/panel/pharmacies'"
              @click="goBack"
              ...
              <Transition name="slide-down">
                <div v-if="showNotifications" class="absolute left-0 mt-2 w-80 bg-white rounded-xl overflow-hidden border border-gray-200">
                  ...
                  <div class="p-3 bg-gray-50 text-center">
                    <NuxtLink 
                      to="/panel/pharmacies/notifications" 
                      @click="showNotifications = false"
...
const menuItems = computed(() => [
  { path: '/panel/pharmacies', label: 'داشبورد', icon: 'ti ti-dashboard', exact: true },
  { path: '/panel/pharmacies/orders', label: 'سفارشات', icon: 'ti ti-shopping-cart', badge: '20' },
  { path: '/panel/pharmacies/products', label: 'محصولات', icon: 'ti ti-shopping-bag' },
  { path: '/panel/pharmacies/reviews', label: 'دیدگاه‌ها', icon: 'ti ti-star', badge: '52' },
  { path: '/panel/pharmacies/financial', label: 'امور مالی', icon: 'ti ti-coins' },
  { path: '/panel/pharmacies/settings', label: 'تنظیمات', icon: 'ti ti-settings' }
])

const bottomNavItems = ref([
  { path: '/panel/pharmacies', label: 'خانه', icon: 'ti ti-home', exact: true },
  { path: '/panel/pharmacies/orders', label: 'سفارشات', icon: 'ti ti-shopping-cart' },
  { path: '/panel/pharmacies/reviews', label: 'دیدگاه‌ها', icon: 'ti ti-star' },
  { path: '/panel/pharmacies/settings', label: 'تنظیمات', icon: 'ti ti-settings' }
])
...
const getPageTitle = () => {
  ...
  if (path === '/panel/pharmacies') return 'داشبورد'
...
const getPageDescription = () => {
  const path = route.path
  if (path === '/panel/pharmacies') return 'مدیریت و نظارت بر عملکرد داروخانه'
