...
export const usePharmacyOrderStore = defineStore("pharmacyOrder", () => {
    // تغییر تایپ به PharmacyOrder برای یکسان‌سازی
    ...
    const stats = computed(() => {
        const allOrders = orders.value;
         ...
         // تفکیک سفارشات بر اساس نوع
        const productOrders = allOrders.filter(o => o.type === 'PRODUCT');
        const pharmacyOrders = allOrders.filter(o => (o.type === 'NONE_PRESCRIPTION'||o.type === 'PRESCRIPTION'));

        return {
            totalOrders: allOrders.length,
            //pendingOrders: allOrders.filter(o => o.status === OrderStatus.CUSTOMER_PENDING).length,
            pendingOrders: allOrders.filter(o => o.status === OrderStatus.CUSTOMER_PAID).length,
            ...
            // --- آمار سفارشات دارویی ---
            pharmacyTotal: pharmacyOrders.length,
            pharmacyPending: pharmacyOrders.filter(o => o.status === OrderStatus.CUSTOMER_PAID).length,
            pharmacyProcessing: pharmacyOrders.filter(o => o.status === OrderStatus.TENANT_PROCESSING).length,
            pharmacyShipped: pharmacyOrders.filter(o => o.status === OrderStatus.TENANT_SHIPPED).length,
            pharmacyDelivered: pharmacyOrders.filter(o => o.status === OrderStatus.CUSTOMER_DELIVERED).length,

            // --- آمار سفارشات محصولات ---
            productTotal: productOrders.length,
            productPending: productOrders.filter(o => o.status === OrderStatus.CUSTOMER_PAID).length,
            productProcessing: productOrders.filter(o => o.status === OrderStatus.TENANT_PROCESSING).length,
            productShipped: productOrders.filter(o => o.status === OrderStatus.TENANT_SHIPPED).length,
            productDelivered: productOrders.filter(o => o.status === OrderStatus.CUSTOMER_DELIVERED).length,
    ...
    /**
     * دریافت لیست سفارشات در انتظار تایید
     */
    /**
     * دریافت لیست سفارشات در انتظار تایید (ساختار سفارشی)
     */
    const pendingOrders = computed(() => {
        return orders.value
            .filter(order => order.status === OrderStatus.CUSTOMER_PAID)
            .map(order => {
                // محاسبه تعداد اقلام
                const itemsCount = order.items.reduce((sum, item) => sum + item.quantity, 0);

                // محاسبه زمان نسبی (مثلاً 5 دقیقه پیش)
                const now = new Date();
                const orderDate = new Date(order.createdAt);
                const diffInSeconds = Math.floor((now.getTime() - orderDate.getTime()) / 1000);
                const diffInMinutes = Math.floor(diffInSeconds / 60);
                const diffInHours = Math.floor(diffInMinutes / 60);
                let timeAgo = 'همین الان';
                if (diffInSeconds > 60) {
                    timeAgo = `${diffInMinutes} دقیقه پیش`;
                }
                if (diffInMinutes > 60) {
                    timeAgo = `${diffInHours} ساعت پیش`;
                }
                ...
                return {
                    ...
                    customerName: order.user
                        ...
                        : 'مشتری ناشناس',
                    phone:order.user?`${order.user.phoneNumber}` : '',
                    itemsCount: itemsCount,
                    items:order.items,
                    shipping:order.metadata?.shippingCost,
    ...
    const recentOrders = computed(() => {
        ...
        return rawOrders.map(order => {
            ...
            const customerName = order.user
                ? `${order.user.firstName || ''} ${order.user.lastName || ''}  ${order.user.fullName || ''}`.trim()
                : 'مشتری ناشناس';
     ...
     */
    const fetchPharmacyOrders = async (page: number = 1, limit: number = 10) => {
        loading.value = true;
        try {
            // ارسال پارامترهای صفحه‌بندی به سرور
            const res = await axios.get<OrdersResponse>("/pharmacy/orders/list", {
                params: {
                    page,
                    limit
                }
    ...
    const updateOrderStatus = async (orderId: string, status: OrderStatus) => {
        ...
        try {
            ...
            await axios.patch(`/pharmacy/orders/${orderId}/status`, { status });
    ...
    const updateOrderMetadata=async(orderId:string, newMetadata:any)=> {
        try {
            // فراخوانی API بک‌اند
            const response = await axios.patch(`/pharmacy/orders/${orderId}/metadata`, {
                metadata: newMetadata
            });

            // آپدیت کردن لیست محلی سفارشات برای نمایش تغییرات در UI
            const index = orders.value.findIndex(o => o.id === orderId);
            if (index !== -1) {
                // ادغام متادیتای جدید با متادیتای قبلی در لیست محلی
                orders.value[index] = {
                    ...orders.value[index],
                    metadata: { ...orders.value[index].metadata, ...newMetadata }
                };
            }
    ...
    const updateTrackingCode = async (orderId: string, trackingCode: string, provider: string = 'manual') => {
        ...
        try {
            ...
            await axios.post(`/pharmacy/orders/${orderId}/tracking`, {
                trackingCode,
                provider
            });
    ...
    const requestShipment = async (orderId: string, provider: string, tenant: TenantInfo) => {
        ...
        try {
            ...
            const origin = {
                ...
                unit: "",
                number: "",
                person_fullname: tenant.ownerName || "مدیر فروشگاه",
            ...
            const scheduleAt = deliveryTimeStandard ? `${deliveryDate} ${deliveryTimeStandard}` : null;
    ...
    /**
     * آمار فروش امروز (تعداد سفارش، تعداد محصول، درآمد)
     */
    const todayStats = computed(() => {
        const now = new Date();
        // شروع امروز (ساعت ۰۰:۰۰:۰۰)
        const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        // پایان امروز (ساعت ۲۳:۵۹:۵۹)
        const endOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1);

        // فیلتر کردن سفارشات امروز
        ...
        // محاسبه تعداد کل اقلام فروخته شده
        const productsCount = todaysOrders.reduce((sum, order) => {
            return sum + order.items.reduce((itemSum, item) => itemSum + item.quantity, 0);
        }, 0);

        // محاسبه کل درآمد
        const revenue = todaysOrders.reduce((sum, order) => sum + (order.totalAmount || 0), 0);
    ...
    /**
     * محاسبه آمار فروش بر اساس بازه زمانی (هفتگی، ماهانه، سه ماهه)
     */
    const statsData = computed(() => {
        ...
        // تابع کمکی برای محاسبه آمار یک بازه خاص
        const calculateStats = (startDate: Date, prevStartDate: Date) => {
            // ۱. فیلتر سفارشات بازه جاری
            const currentOrders = orders.value.filter(order => {
                const orderDate = new Date(order.createdAt);
            ...
            // ۲. فیلتر سفارشات بازه قبلی (برای محاسبه رشد)
            const prevOrders = orders.value.filter(order => {
                const orderDate = new Date(order.createdAt);
                const isInPrevRange = orderDate >= prevStartDate && orderDate < startDate;
                const isNotCancelled = ![OrderStatus.TENANT_CANCELLED, OrderStatus.CUSTOMER_CANCELLED].includes(order.status);
                return isInPrevRange && isNotCancelled;
            });

            // ۳. محاسبه مقادیر مالی
        ...
        // محاسبه تاریخ شروع بازه‌های قبلی (برای مقایسه رشد)
        ...
        prevQuarterStart.setMonth(now.getMonth() - 6);

        return {
            weekly: calculateStats(startOfWeek, prevWeekStart),
            monthly: calculateStats(startOfMonth, prevMonthStart),
            quarterly: calculateStats(startOfQuarter, prevQuarterStart)
        };
    });
    ...
    const topSellingProducts = computed(() => {
        ...
        orders.value.forEach(order => {
            ...
            order.items.forEach(item => {
                ...
                const productName = item.marketProduct?.product?.name || item.pharmacyMedicine?.medicine.name ;
                const category = item.marketProduct?.product?.categoryBreadcrumb || item.pharmacyMedicine?.medicine?.categoryBreadcrumb;
