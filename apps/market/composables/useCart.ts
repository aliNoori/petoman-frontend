...
if (typeof window !== 'undefined') {
    const savedCart = localStorage.getItem('pet-market-cart')
    if (savedCart) {
        try {
    ...
    const savedDiscount = localStorage.getItem('pet-market-discount')
    if (savedDiscount) {
        try {
...
watch(cartItems, (newVal) => {
    if (typeof window !== 'undefined') {
        localStorage.setItem('pet-market-cart', JSON.stringify(newVal))
    }
}, {deep: true})
...
watch(appliedDiscount, (newVal) => {
    if (typeof window !== 'undefined') {
        if (newVal) {
            localStorage.setItem('pet-market-discount', JSON.stringify(newVal))
        } else {
            localStorage.removeItem('pet-market-discount')
        }
    }
})
