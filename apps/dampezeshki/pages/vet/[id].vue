  ...
  <div class="min-h-screen bg-gray-50 font-vazir">
    ...
    <div class="container mx-auto px-4 pt-6">
      <nav class="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <NuxtLink to="/vets" class="hover:text-sky-600 transition-colors">دامپزشکان</NuxtLink>
        <i class="ti ti-chevron-left text-xs"></i>
        <span class="text-sky-600 font-medium">{{ vet.name }}</span>
      </nav>
    </div>

    <!-- Hero Header - با رادیوس و عرض container -->
    <div class="container mx-auto px-4 pb-6">
      <div class="relative bg-gradient-to-br from-sky-500 via-sky-600 to-blue-600 rounded-2xl overflow-hidden">
          ...
          <div class="px-6 py-4">
            <nav class="flex items-center justify-between text-white">
              ...
              <div class="flex items-center gap-3">
                <button @click="shareVet" class="hover:bg-white/10 p-2 rounded-lg transition-colors"
            ...
            <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
              <div class="relative">
                <div
                  ...
                  <span v-else class="text-4xl font-bold text-white relative z-10">{{ vet.avatar }}</span>

                </div>
                <div v-if="vet.isOnline"
                     class="absolute bottom-2 right-2 w-5 h-5 bg-green-500 rounded-full border-2 border-white"></div>
              </div>
              <div class="flex-1 text-center md:text-right">
                <h1 class="text-2xl md:text-3xl font-bold text-white mb-2">{{ vet.name }}</h1>
                <p class="text-sky-100 mb-3">{{ vet.specialty }}</p>
                <div class="flex flex-wrap items-center justify-center md:justify-start gap-3">
                  <div class="flex items-center gap-2 bg-white/20 px-3 py-1 rounded-full text-sm">
                    ...
                    <span class="text-white font-semibold">{{ vet.rating }}</span>
                    <span class="text-white/70">({{ vet.reviewCount }} نظر)</span>
                  </div>
                  <div :class="vet.isOnline ? 'bg-green-500' : 'bg-gray-500'"
                       class="px-3 py-1 rounded-full text-white text-sm font-medium">
                    {{ vet.isOnline ? 'آنلاین' : 'آفلاین' }}
                  </div>
                  <div class="flex items-center gap-2 text-white/90 text-sm">
                    <i class="ti ti-certificate"></i>
                    <span>{{ vet.experience }} سال تجربه</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <!-- تب‌ها - درباره دکتر / خدمات / نظرات کاربران -->
    ...
    <div class="container mx-auto px-4 py-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <!-- انتخاب تاریخ و ساعت - فقط برای خدمات نوبت‌دار -->
          <div v-if="selectedService && !selectedService.isInstant"
            ...
            <div v-if="selectedDate" class="bg-sky-50 rounded-xl p-3 flex items-center gap-3 mb-6">
              <i class="ti ti-calendar-check text-sky-500 text-xl"></i>
              <div>
                <span class="text-sm text-gray-600">تاریخ انتخاب شده:</span>
                <span class="font-bold text-sky-700 mr-2">{{ selectedDateDisplay }}</span>
              </div>
            </div>
            ...
            <div v-if="selectedDate && !selectedDate.isFriday">
              <h3 class="font-medium text-gray-700 mb-3 flex items-center gap-2">
                <i class="ti ti-clock text-sky-500"></i>
                ساعت مراجعه
              ...
              <div class="flex gap-2 mb-4">
                <button
                    ...
                    :class="['px-4 py-2 rounded-lg font-medium transition-all', timePeriod === 'morning' ? 'bg-sky-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']"
                >
                  صبح
                </button>
                <button
                    ...
                    :class="['px-4 py-2 rounded-lg font-medium transition-all', timePeriod === 'afternoon' ? 'bg-sky-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']"
                >
                  عصر
                </button>
              </div>
              ...
              <div class="grid grid-cols-4 md:grid-cols-6 gap-2">
                ...
                <button
                    ...
                    :title="time.disabledReason || ''"
                    :class="[
                    'p-3 rounded-xl text-center transition-all font-medium relative',
                    !time.available
                      ? time.reason === 'pending'
                ...
                >
                  ...
                  <!-- نشانگر pending -->
                  <span
                      v-if="!time.available && time.reason === 'pending'"
          ...
          <!-- ستون کناری - ظرفیت و صف -->
          <div v-if="selectedService?.isInstant" class="mt-4 p-4 rounded-lg"
               :class="vetStore.capacityStatus.isOnline ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'">

            <!-- دکتر آنلاین و آزاد -->
            <div v-if="vetStore.capacityStatus.isOnline && vetStore.capacityStatus.available"
                 class="flex items-center gap-3 text-green-700">
              <div class="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
                <i class="ti ti-circle-check text-xl"></i>
              </div>
              <div>
                <p class="font-bold">دکتر آنلاین و آماده پاسخگویی است</p>
                <p class="text-sm text-green-600/80">همین الان می‌توانید مشاوره بگیرید</p>
              </div>
            </div>

            <!-- دکتر آنلاین ولی مشغول -->
            <div v-else-if="vetStore.capacityStatus.isOnline && !vetStore.capacityStatus.available">
              <div class="flex items-center gap-3 text-orange-700 mb-4">
                ...
                <div class="flex-1">
                  <p class="font-bold">دکتر در حال مشاوره است</p>
                  <p class="text-sm text-orange-600/80">با پرداخت در صف انتظار قرار می‌گیرید</p>
                </div>
              </div>

              <!-- چک‌باکس پذیرش صف -->
              <label class="flex items-center gap-3 p-3 bg-white rounded-xl border-2 cursor-pointer transition-all"
                     :class="acceptQueue ? 'border-orange-400 bg-orange-50' : 'border-gray-200 hover:border-orange-300'">
                <input
                    type="checkbox"
                    v-model="acceptQueue"
                    class="w-5 h-5 rounded border-gray-300 text-orange-500 focus:ring-orange-400 cursor-pointer"
                >
                <div class="flex-1">
                  <p class="font-bold text-gray-900">می‌پذیرم در صف منتظر بمانم</p>
                  <p class="text-xs text-gray-500 mt-0.5">
                    اگر نوبت الان نباشد، در صف قرار می‌گیرم
                  </p>
                </div>
              </label>
            </div>
            ...
            <div v-else class="flex items-center gap-3 text-red-700">
              ...
              <div>
                <p class="font-bold">دکتر در حال حاضر آنلاین نیست</p>
                <p class="text-sm text-red-600/80">لطفاً بعداً مراجعه کنید</p>
              </div>
            </div>

            <!-- اطلاعات صف بعد از پرداخت -->
            <div v-if="queueInfo" class="mt-4 pt-4 border-t border-green-200">
              <div class="bg-white rounded-xl p-4">
                <div class="flex items-center justify-between mb-3">
                  <div class="flex items-center gap-2">
                    <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                    <span class="font-bold text-green-700">شما در صف هستید</span>
                  </div>
                  <button
                      @click="handleLeaveQueue"
                      class="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
                  >
                    <i class="ti ti-x ml-1"></i>خروج از صف
                  </button>
                </div>

                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-gray-50 rounded-lg p-3 text-center">
                    <span class="text-2xl font-bold text-sky-600">{{ queueInfo.queueNumber }}</span>
                    <span class="text-xs text-gray-500 block">موقعیت شما</span>
                  </div>
                  <div class="bg-gray-50 rounded-lg p-3 text-center">
                    <span class="text-2xl font-bold text-orange-600">{{ queueInfo.estimatedWaitMinutes }}</span>
                    <span class="text-xs text-gray-500 block">دقیقه انتظار</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div id="about" ref="aboutSection" class="bg-white rounded-xl border border-gray-200 p-6 scroll-mt-20">
            ...
            <p class="text-gray-700 leading-relaxed mb-4">{{ vet.about }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                <h4 class="font-medium text-gray-900 mb-2 flex items-center gap-2">
                  <i class="ti ti-school text-sky-500"></i>
                  تحصیلات
                </h4>
                <ul v-if="vet.education && vet.education.length > 0" class="space-y-2 text-sm text-gray-600">
                  <li v-for="edu in vet.education" :key="edu">• {{ edu }}</li>
                </ul>
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                <h4 class="font-medium text-gray-900 mb-2 flex items-center gap-2">
                  <i class="ti ti-certificate text-sky-500"></i>
                  تخصص‌ها
                </h4>
                <ul v-if="vet.specialties && vet.specialties.length > 0" class="space-y-2 text-sm text-gray-600">
                  <li v-for="spec in vet.specialties" :key="spec">• {{ spec }}</li>
                </ul>
          ...
          <div id="services" ref="servicesSection" class="bg-white rounded-xl border border-gray-200 p-6 scroll-mt-20">
            <!-- بخش خدمات دامپزشک -->
            <div v-if="vet.availableServices && vet.availableServices.length > 0" class="mt-6">
              ...
              <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                <div
                    v-for="(service, index) in vet.availableServices"
                    :key="index"
                    class="group bg-white border border-gray-100 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-300 flex flex-col justify-between"
                >
                  <!-- آیکون و عنوان -->
                  <div class="flex items-start gap-3 mb-2">
                    <div
                        class="shrink-0 w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
                      <i class="ti ti-stethoscope text-xl"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                      <h4 class="text-sm font-semibold text-gray-800 truncate" :title="service.name || service">
                  ...
                  <!-- خط جداکننده -->
                  <div class="border-b border-gray-50 my-2"></div>

                  <!-- بخش قیمت -->
                  <div class="flex items-center justify-between mt-1">
                    <span class="text-xs text-gray-500">تعرفه:</span>
                    <span class="text-sm font-bold text-gray-900" v-if="service.price">
                      {{ Number(service.price).toLocaleString() }} <span
                        class="text-xs font-normal text-gray-500">تومان</span>
                    </span>
                    <span class="text-xs text-gray-400 italic" v-else>
                    توافقی
                  </span>
                  </div>
                </div>
          ...
          <div id="location" class="bg-white rounded-xl border border-gray-200 p-6 scroll-mt-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                <i class="ti ti-map-pin text-sky-500 text-xl mt-0.5"></i>
                <div class="flex-1">
                  <p class="font-medium text-gray-900 mb-1">آدرس</p>
                  <p class="text-gray-600 text-sm">{{ vet.address }}</p>
                  <a
                      :href="`https://maps.google.com/?q=${vet.lat},${vet.lng}`"
                      target="_blank"
                      class="text-sky-600 text-sm mt-2 inline-flex items-center gap-1 hover:underline"
                  >
              ...
              <!--
                            &lt;!&ndash; ساعات کاری &ndash;&gt;
                            <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                              <i class="ti ti-clock text-sky-500 text-xl mt-0.5"></i>
                              <div class="flex-1">
                                <p class="font-medium text-gray-900 mb-2">ساعات کاری</p>
                                <div class="space-y-1 text-sm">
                                  <div v-for="schedule in vet.workingHours" :key="schedule.day" class="flex justify-between">
          ...
          <div id="reviews" ref="reviewsSection" class="bg-white rounded-xl border border-gray-200 p-6 scroll-mt-20">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                <i class="ti ti-message-circle text-sky-500"></i>
                نظرات کاربران
              ...
              <div class="flex items-center gap-2 bg-amber-50 px-3 py-1 rounded-full">
                ...
                <span class="font-bold text-amber-700">{{ vet.rating }}</span>
              </div>
            </div>
              ...
              <div v-for="review in paginatedReviews" :key="review.id"
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <!-- پاسخ دکتر -->
                    <div v-if="review.doctorReply" class="bg-sky-50 rounded-lg p-4 mt-3">
                      <div class="flex items-start gap-2 mb-2">
                        <i class="ti ti-stethoscope text-sky-600"></i>
            ...
            <div v-if="reviews.length > reviewsPerPage && !showAllReviews" class="mt-6 text-center">
              <button
                  ...
                  class="px-6 py-3 bg-sky-50 hover:bg-sky-100 text-sky-600 font-medium rounded-xl inline-flex items-center gap-2 transition-colors"
              >
                مشاهده همه نظرات
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-6 sticky top-24">
            ...
            <div class="space-y-3 mb-6">
              <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-gray-900">{{ vet.name }}</span>
              </div>
              ...
              <div v-if="selectedService" class="flex justify-between text-sm border-t pt-3">
                ...
                <span class="font-bold text-sky-600 text-lg">{{ selectedService.price }} تومان</span>
              </div>
            </div>
            ...
            <div v-if="selectedService && selectedService.isInstant" class="mb-6">
              ...
              <div class="space-y-3">
                ...
                <label
                    ...
                    :class="selectedPaymentMethod === 'wallet' ? 'border-sky-500 bg-sky-50' : 'border-gray-200 hover:bg-gray-50'"
                >
                ...
                <label
                    ...
                    :class="selectedPaymentMethod === 'gateway' ? 'border-sky-500 bg-sky-50' : 'border-gray-200 hover:bg-gray-50'"
                >
                  <input
                      type="radio"
                      v-model="selectedPaymentMethod"
              ...
              <div v-if="selectedPaymentMethod === 'wallet' && !canUseWallet"
                   class="mt-2 text-xs text-red-500 flex items-center">
            ...
            <button
                ...
                :disabled="!canProceed || (selectedPaymentMethod === 'wallet' && !canUseWallet)||!selectedPetId"
                class="w-full py-3 px-4 rounded-xl font-medium transition-colors bg-sky-600 text-white hover:bg-sky-700 disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-not-allowed"
            >
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 relative">
          ...
          <div class="text-center mb-6">
            <div class="w-16 h-16 bg-sky-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <i class="ti ti-user-circle text-3xl text-sky-600"></i>
            </div>
          ...
          <div class="space-y-4">
            <NuxtLink
                :to="`${config.public.authBaseUrl}?redirect=${encodeURIComponent(currentUrl)}`"
                class="w-full py-3 px-4 bg-sky-600 text-white rounded-xl font-medium hover:bg-sky-700 transition-colors flex items-center justify-center gap-2"
            >
        ...
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
          ...
          <div class="space-y-4 mb-6">
            ...
            <textarea
                ...
                rows="3"
                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-100 outline-none resize-none"
          ...
          <div class="bg-gray-50 rounded-xl p-4 mb-6">
            <div class="flex justify-between items-center">
              ...
              <span class="text-xl font-bold text-sky-600">{{ selectedService?.price }} تومان</span>
            </div>
          </div>
    ...
    <!-- مدال موفقیت -->
    <Teleport to="body">
      <div v-if="showSuccessModal" class="fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center p-4">
        <div v-if="vet && vet.name" class="bg-white rounded-2xl max-w-md w-full p-6 text-center">
          ...
          <p class="text-gray-600 mb-2">کد رهگیری: <span class="font-bold text-sky-600 font-english">{{
              trackingCode
            }}</span></p>
          ...
          <div v-else-if="selectedService?.id === 'phone-scheduled'" class="my-6">
            <p class="text-sm text-gray-600">دکتر در زمان انتخابی شما (<span
                class="font-bold text-purple-600">{{ getPhoneTimeDisplay() }}</span>) با شما تماس خواهد گرفت</p>
          </div>
          ...
          <div class="flex gap-3">
            <NuxtLink to="/profile"
                      class="flex-1 py-3 px-4 bg-sky-500 text-white rounded-xl font-medium">
              مشاوره‌های من
            </NuxtLink>
    ...
    <!-- مدال انتخاب زمان تماس تلفنی -->
    <Teleport to="body">
      <div v-if="showPhoneTimeModal"
           class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 z-[1000]">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 relative max-h-[90vh] overflow-y-auto">
          ...
          <!-- هدر -->
          <div class="bg-purple-600 rounded-xl p-4 text-white mb-6 -mx-2 -mt-2">
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
          ...
          <div class="space-y-3 mb-6">
            ...
            <!-- گزینه ۱۵ دقیقه -->
            <div
                v-if="vet.rawPhoneSchedule?.min15?.enabled"
                @click="phoneCallOption = 'min15'"
                class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all"
                :class="phoneCallOption === 'min15' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300'"
            >
              <div class="w-10 h-10 rounded-full flex items-center justify-center"
                   :class="phoneCallOption === 'min15' ? 'bg-purple-500 text-white' : 'bg-gray-100'">
                <i class="ti ti-clock text-xl"></i>
              </div>
              <div class="flex-1">
              ...
              <div class="text-left">
                <p class="font-bold text-purple-600">{{ formatPrice(vet.rawPhoneSchedule?.min15.price) }}</p>
                <p class="text-xs text-gray-500">تومان</p>
              </div>
              <i v-if="phoneCallOption === 'min15'" class="ti ti-check text-purple-500 text-xl"></i>
            </div>

            <!-- گزینه ۳۰ دقیقه -->
            <div
                v-if="vet.rawPhoneSchedule?.min30?.enabled"
                @click="phoneCallOption = 'min30'"
                class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all"
                :class="phoneCallOption === 'min30' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300'"
            >
              <div class="w-10 h-10 rounded-full flex items-center justify-center"
                   :class="phoneCallOption === 'min30' ? 'bg-purple-500 text-white' : 'bg-gray-100'">
                <i class="ti ti-clock text-xl"></i>
              </div>
              <div class="flex-1">
              ...
              <div class="text-left">
                <p class="font-bold text-purple-600">{{ formatPrice(vet.rawPhoneSchedule?.min30.price) }}</p>
                <p class="text-xs text-gray-500">تومان</p>
              </div>
              <i v-if="phoneCallOption === 'min30'" class="ti ti-check text-purple-500 text-xl"></i>
            </div>

            <!-- گزینه ۱ ساعت -->
            <div
                v-if="vet.rawPhoneSchedule?.hour1?.enabled"
                @click="phoneCallOption = 'hour1'"
                class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all"
                :class="phoneCallOption === 'hour1' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300'"
            >
              <div class="w-10 h-10 rounded-full flex items-center justify-center"
                   :class="phoneCallOption === 'hour1' ? 'bg-purple-500 text-white' : 'bg-gray-100'">
              ...
              <div class="text-left">
                <p class="font-bold text-purple-600">{{ formatPrice(vet.rawPhoneSchedule?.hour1.price) }}</p>
                <p class="text-xs text-gray-500">تومان</p>
              </div>
              <i v-if="phoneCallOption === 'hour1'" class="ti ti-check text-purple-500 text-xl"></i>
            </div>

            <!-- گزینه زمان دلخواه -->
            <div
                v-if="vet.rawPhoneSchedule?.custom?.enabled"
                @click="phoneCallOption = 'custom'"
                class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all"
                :class="phoneCallOption === 'custom' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300'"
            >
              <div class="w-10 h-10 rounded-full flex items-center justify-center"
                   :class="phoneCallOption === 'custom' ? 'bg-purple-500 text-white' : 'bg-gray-100'">
              ...
              <div class="text-left">
                <p class="font-bold text-purple-600">{{ formatPrice(vet.rawPhoneSchedule?.custom.price) }}</p>
                <p class="text-xs text-gray-500">تومان</p>
              </div>
              <i v-if="phoneCallOption === 'custom'" class="ti ti-check text-purple-500 text-xl"></i>
            </div>
          ...
          <div class="bg-gray-50 rounded-xl p-4 mb-6">
            <div class="flex justify-between items-center">
              <span class="text-gray-600">مبلغ قابل پرداخت</span>
              <span class="text-xl font-bold text-purple-600">{{ getPhoneCallPrice() }} تومان</span>
            </div>
          </div>
          ...
          <button
              ...
              class="w-full py-3 px-4 bg-purple-600 text-white rounded-xl font-medium hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
    ...
    <Teleport to="body">
      <div v-if="showReviewModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
        ...
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-gray-900">ثبت نظر برای {{ vet.name }}</h3>
          ...
          <div class="mb-6">
            ...
            <textarea
                ...
                placeholder="نظر خود را درباره این دامپزشک بنویسید..."
            ></textarea>
          </div>
          ...
          <button
              ...
              class="w-full py-3 bg-sky-500 text-white rounded-lg font-medium hover:bg-sky-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ثبت نظر
          </button>
        </div>
...
// 3. در تابع handleAddPet، اگر می‌خواهید تصویر را نیز به سرور بفرستید، ساختار درخواست را به شکل زیر تغییر دهید:
const handleAddPet = async () => {
...
// 1. اضافه کردن متغیرهای جدید برای محاسبه دقیق
...
// 2. محاسبه قیمت خام سرویس انتخاب شده
const selectedServiceRawPrice = computed(() => {
  if (!selectedService.value) return 0;
  // اگر قیمت به صورت رشته فارسی است (مثل ۳۳٬۳۳۳)، آن را تمیز می‌کنیم
  let priceStr = String(selectedService.value.price || 0);
  // تبدیل اعداد فارسی به انگلیسی و حذف جداکننده‌ها
  priceStr = priceStr.replace(/[۰-۹]/g, d => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d));
  return Number(priceStr.replace(/[^0-9.]/g, ''));
});

// 3. کمپوتد جدید برای بررسی امکان پرداخت با کیف پول
const canUseWallet = computed(() => {
  if (!selectedService.value) return false;
  return walletBalance.value >= selectedServiceRawPrice.value;
});
...
// گرفتن tenantId از route
const vetId = computed(() => String(route.params.id))
const queueInfo = computed(() => vetStore.queueInfo);

// Handler برای خروج از صف
const handleLeaveQueue = async () => {
  if (!vetStore.queueInfo?.orderId) return

  const result = await vetStore.leaveQueue(vetStore.queueInfo.orderId)
...
// اضافه کنید به watch موجود:
watch(
    () => selectedService.value,
    async (newService) => {
      if (newService && vetId.value) {
        await vetStore.checkVetAvailability(vetId.value, newService.id)
        await vetStore.startQueuePolling(vetId.value)
...
onMounted(async () => {
  ...
  // بررسی وضعیت اولیه
  await vetStore.checkVetAvailability(vetId.value)

  // شروع polling دوره‌ای
  await vetStore.startAvailabilityPolling(vetId.value)

  await vetStore.setOnTurnReachedCallback(() => {
    // نوبت کاربر رسیده!
...
// تب‌ها
const tabs = [
  {id: 'about', name: 'درباره دکتر', icon: 'ti ti-user'},
  {id: 'services', name: 'خدمات', icon: 'ti ti-medical-cross'},
  {id: 'reviews', name: 'نظرات کاربران', icon: 'ti ti-message-circle'}
]
const settings = computed(() => vet.settings)

// نظرات کاربران (داینامیک از استور)
...
// ===== اضافه کردن: دریافت order های در حال پرداخت (PENDING) =====
const pendingOrders = computed(() => {
  if (!appointmentStore.timeAppointments.value) return []
  return appointmentStore.timeAppointments.value.filter((apt: any) =>
      apt.order?.status === 'CUSTOMER_PENDING' ||
      apt.order?.status === 'PENDING' ||
      apt.order?.status === 'WAITING_FOR_PAYMENT'
  )
})

// ===== اصلاح checkSlotAvailability برای بررسی order های PENDING =====
const checkSlotAvailability = (time: string, dateStr: string) => {
  const timeInMinutes = timeToMinutes(time)

  // ── ۱. بررسی time off blocks (همون که بود) ──
  const isTimeOff = appointmentStore.timeOffBlocks.some((block: any) => {
  ...
  // ── ۲. بررسی appointment های تأیید شده (همون که بود) ──
  const isBooked = appointmentStore.timeAppointments.some((apt: any) => {
    ...
    return aptDate === dateStr && reservedTime === time
  })
  if (isBooked) return {available: false, reason: 'booked'}

  // ── ۳. بررسی order های در حال پرداخت (جدید!) ──
  // این بخش جلوی race condition رو می‌گیره
  const isPending = pendingOrders.value.some((order: any) => {
    const payment = order.payment
    if (!payment) return false
    const reservedDate = payment.metadata?.reservedTime?.date
    const reservedTime = payment.metadata?.reservedTime?.time
    if (!reservedDate || !reservedTime) return false
    const orderDate = reservedDate.replace(/-/g, '/')
    // فقط آنهایی که هنوز PENDING هستند (نه CANCELLED)
    const isNotCancelled = order.status !== 'CUSTOMER_CANCELLED' &&
        order.status !== 'CANCELLED' && order.status !== 'TENANT_CANCELLED'
    return orderDate === dateStr && reservedTime === time && isNotCancelled
  })
  if (isPending) return {available: false, reason: 'pending'}
...
const filteredTimes = computed(() => {
  ...
  console.log('sd',selectedDate.value)
  ...
  return baseTimes.map(slot => {
    ...
    return {
      ...
      disabledReason: status.reason === 'booked' ? 'رزرو شده' :
          status.reason === 'pending' ? 'در حال پرداخت توسط دیگری' :
              status.reason === 'time-off' ? 'زمان استراحت دکتر' : null
    }
  })
})
...
const canProceed = computed(() => {
  ...
  if (selectedService.value.isInstant) {
    // اگه phone-scheduled هست، باید phoneCallOption انتخاب شده باشه
...
console.log('cp',canProceed.value)
...
const vet = computed(() => {
  // پیدا کردن کلینیک در لیست دامپزشکان استور بر اساس ID از URL
  const vetData = tenantStore.vets.find((v: any) => v.id === route.params.id);
  ...
  const services = vetData.tenantServices || [];
  ...
  const lat = vetData.tenantAddress.location?.lat || 35.7155;
  const lng = vetData.tenantAddress.location?.lng || 51.4085;
  ...
  return {
    id: vetData.id,
    name: vetData.ownerName || vetData.name || clinicInfo.name || clinicInfo.ownerName || 'نامشخص',
    //logo: clinicInfo.logo,
    logo: resolveUrl(clinicInfo.logo),
    avatar: vetData.name ? vetData.name.charAt(0) : '؟',
    specialty: vetData.specialty || 'دامپزشک',
    specialties: clinicInfo.specialties,
    education: clinicInfo.education,
    availableServices: services
        .filter((s) => s.status === 'active')
        .map((s) => ({
          name: s.description || s.name, // نام سرویس
          price: s.price
        })),
    city: vetData.city,
    address: [
      vetData.tenantAddress?.detailedAddress?.road,
      vetData.tenantAddress?.detailedAddress?.neighbourhood,
      vetData.tenantAddress?.detailedAddress?.suburb
    ]
    ...
    rating: parseFloat(vetData.rating) || 0,
    reviewCount: vetData.reviewsCount || 0,
    isOnline: tenantStore.calculateOnlineStatus(vetData.id, socketStore.onlineUsers) || vetData.isOnline,
    isSuspended: vetData.isSuspended, // وضعیت تعلیق
...
const services = computed(() => {
  if (!vet.value.rawVisitPricing) return [];

  const vp = vet.value.rawVisitPricing;
  const ps = vet.value.rawPhoneSchedule;
...
const getPhoneTimeDisplay = () => {
  if (!phoneCallOption.value) return '';
  const ps = vet.value.rawPhoneSchedule;
  if (phoneCallOption.value === 'min15' && ps?.min15) return `۱۵ دقیقه دیگر`;
  if (phoneCallOption.value === 'min30' && ps?.min30) return `۳۰ دقیقه دیگر`;
  if (phoneCallOption.value === 'hour1' && ps?.hour1) return `۱ ساعت دیگر`;
  if (phoneCallOption.value === 'custom' && ps?.custom) return `زمان دلخواه`;
  return '';
};
...
const proceedToPayment = () => {
  ...
  if (selectedService.value?.isInstant) {
    ...
    const status = checkSlotAvailability(
        selectedTime.value,
        selectedDate.value.fullDate
    )

    if (!status.available) {
      // اگه تداخل پیدا شد، به کاربر اطلاع بده
      //alert('متأسفانه این ساعت هم‌اکنون رزرو شده است. لطفاً ساعت دیگری انتخاب کنید.')
      $toast('متأسفانه این ساعت هم‌اکنون رزرو شده است. لطفاً ساعت دیگری انتخاب کنید.', 'warning', 5000)
      return
    }
  } else {
...
const selectedAddress = ref<{ id: string; label: string } | null>(null);
  ...
  try {
    ...
    if (selectedPaymentMethod.value === 'wallet') {
      if (!canUseWallet) {
        ...
        $toast('موجودی کیف پول شما برای پرداخت این نوبت کافی نیست. لطفاً موجودی خود را افزایش دهید.', 'warning', 5000)
    ...
    if (selectedService.value?.id === 'home') {
      ...
      if (!addressId) {
        ...
        $toast('لطفاً برای ویزیت در منزل، آدرس خود را انتخاب کنید.', 'info', 5000)
        return;
      }
    }
    ...
    const orderPayload = {
      tenantId: vet.value.id, // شناسه کلینیک
      acceptQueue: acceptQueue.value || false,
    ...
    const response = await paymentStore.submitVetOrder(orderPayload);
...
const initMap = async () => {
  ...
  map = L.map(mapContainer.value, {
    center: [vet.value.lat, vet.value.lng],
    zoom: 16,
    zoomControl: false,
...
onMounted(async () => {
  ...
  await appointmentStore.startSlotPolling(String(route.params.id))
...
onUnmounted(() => {
  ...
  vetStore.cleanup() // ← پاکسازی polling ها

  appointmentStore.cleanup()
})

// Share vet profile
const shareVet = async () => {
  const shareData = {
    title: vet.value.name,
    text: `${vet.value.name} - ${vet.value.specialty}`,
...
const submitReview = () => {
  ...
  // Update vet review count
  vet.value.reviewCount++
  ...
  $toast('نظر شما با موفقیت ثبت شد!', 'success', 5000)
}
...
watch(showSuccessModal, (isOpen) => {
  if (isOpen) {
    ...
    if (selectedService.value?.id === 'text') {
      handleRedirectToChat(route.query);
...
const handleRedirectToChat = (data: any) => {
  ...
  navigateTo({
    path: `/chat/${params.consultationId}`, // یا آدرس دقیق صفحه چت شما
    query: params
  });
};
...
useHead({
  title: computed(() => `${vet.value?.name || 'در حال بارگذاری'} - رزرو نوبت | پتومن`),
  meta: [
    {
      name: 'description',
      content: computed(() => `رزرو نوبت از ${vet.value?.name || 'این دامپزشک'}`)
    }
  ]
})
</script>
