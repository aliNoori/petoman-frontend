        ...
        <div>
          ...
          <div class="relative">
            <button
              ...
              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-white hover:border-blue-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all flex items-center justify-between text-right"
            >
              ...
              <div v-if="showStatusDropdown" class="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
                <ul class="py-2">
                  <li 
                    ...
                    :class="{ 'bg-blue-50': filters.status === '' }"
                  >
                    <span :class="filters.status === '' ? 'text-blue-700 font-medium' : 'text-gray-700'">همه</span>
                    <i v-if="filters.status === ''" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectStatusFilter('processing')"
                    class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': filters.status === 'processing' }"
                  >
                    <span :class="filters.status === 'processing' ? 'text-blue-700 font-medium' : 'text-gray-700'">در انتظار پرداخت</span>
                    <i v-if="filters.status === 'processing'" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectStatusFilter('shipped')"
                    class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': filters.status === 'shipped' }"
                  >
                    <span :class="filters.status === 'shipped' ? 'text-blue-700 font-medium' : 'text-gray-700'">در حال ارسال</span>
                    <i v-if="filters.status === 'shipped'" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectStatusFilter('delivered')"
                    class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': filters.status === 'delivered' }"
                  >
                    <span :class="filters.status === 'delivered' ? 'text-blue-700 font-medium' : 'text-gray-700'">تحویل شده</span>
                    <i v-if="filters.status === 'delivered'" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectStatusFilter('cancelled')"
                    class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': filters.status === 'cancelled' }"
                  >
                    <span :class="filters.status === 'cancelled' ? 'text-blue-700 font-medium' : 'text-gray-700'">لغو شده</span>
                    <i v-if="filters.status === 'cancelled'" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectStatusFilter('rejected')"
                    class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': filters.status === 'rejected' }"
                  >
                    <span :class="filters.status === 'rejected' ? 'text-blue-700 font-medium' : 'text-gray-700'">رد شده</span>
                    <i v-if="filters.status === 'rejected'" class="ti ti-check text-blue-600"></i>
                  </li>
                </ul>
              </div>
            </Transition>
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input 
              ...
              class="w-full pr-10 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
          </div>
        </div>
      </div>
            ...
            <tr v-for="order in filteredOrders" :key="order.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4">
                <div class="flex flex-col gap-1">
                  ...
                  <p v-if="order.items.length > 2" class="text-xs text-blue-600 font-medium">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2 relative">
            ...
            <button
              ...
              class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm bg-white hover:border-blue-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all flex items-center gap-2"
            >
              <span>{{ pageSize }}</span>
              ...
              <div v-if="showPageSizeDropdown" class="absolute bottom-full left-0 mb-2 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden min-w-[80px]">
                <ul class="py-2">
                  <li 
                    ...
                    :class="{ 'bg-blue-50': pageSize === 5 }"
                  >
                    <span :class="pageSize === 5 ? 'text-blue-700 font-medium' : 'text-gray-700'">5</span>
                    <i v-if="pageSize === 5" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectPageSize(10)"
                    class="px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': pageSize === 10 }"
                  >
                    <span :class="pageSize === 10 ? 'text-blue-700 font-medium' : 'text-gray-700'">10</span>
                    <i v-if="pageSize === 10" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectPageSize(20)"
                    class="px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': pageSize === 20 }"
                  >
                    <span :class="pageSize === 20 ? 'text-blue-700 font-medium' : 'text-gray-700'">20</span>
                    <i v-if="pageSize === 20" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectPageSize(50)"
                    class="px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': pageSize === 50 }"
                  >
                    <span :class="pageSize === 50 ? 'text-blue-700 font-medium' : 'text-gray-700'">50</span>
                    <i v-if="pageSize === 50" class="ti ti-check text-blue-600"></i>
                  </li>
                  <li 
                    @click="selectPageSize(100)"
                    class="px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between"
                    :class="{ 'bg-blue-50': pageSize === 100 }"
                  >
                    <span :class="pageSize === 100 ? 'text-blue-700 font-medium' : 'text-gray-700'">100</span>
                    <i v-if="pageSize === 100" class="ti ti-check text-blue-600"></i>
                  </li>
                </ul>
              </div>
            </Transition>
...
// استفاده از notification (ساده‌سازی شده)
const notification = {
  show: (options) => {
...
const printOrder = (order) => {
  ...
  try {
    ...
    router.push({
      path: '/panel/pet-shops/products/print-invoice',
