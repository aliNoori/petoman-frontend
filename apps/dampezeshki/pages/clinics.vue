  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="container mx-auto px-4 pt-6">
      ...
      <div class="flex items-center gap-2 text-gray-500 text-sm mb-4">
        ...
        <span class="text-teal-600 font-medium">کلینیک‌ها</span>
      </div>

      <!-- Page Header - With Radius -->
      <div class="bg-gradient-to-br from-teal-500 to-emerald-600 rounded-2xl p-6 mb-6 shadow-lg shadow-teal-500/10">
        <!-- Title Row -->
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div class="flex items-center gap-4">
            <div class="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
              <i class="ti ti-building-hospital text-2xl text-white"></i>
            </div>
            <div>
              <h1 class="text-2xl font-bold text-white">کلینیک‌های پتومن</h1>
              <p class="text-teal-100 text-sm">شبکه معتبر کلینیک‌های دامپزشکی</p>
            </div>
          </div>
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <section class="mb-12">
        ...
        <!-- نمایش گرید کارت‌ها اگر کلینیک موجود باشد -->
        <div v-if="nearby.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          <ClinicCard
            v-for="clinic in nearby.slice(0, 5)"
            :key="clinic.id"
            :clinic="clinic"
            :distance-km="getClinicDistance(clinic)"
          />
        </div>

        <!-- پیام عدم وجود کلینیک -->
        <div v-else class="text-center py-12 bg-gray-50 rounded-xl">
          <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 border border-gray-200">
            <i class="ti ti-building-hospital text-3xl text-gray-400"></i>
          </div>
          <h3 class="text-xl font-semibold text-gray-900 mb-2">کلینیکی یافت نشد</h3>
          <p class="text-gray-600 mb-6">در {{ selectedCity }} کلینیکی ثبت نشده</p>
          <button
      ...
      <section class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          <ClinicCard
            v-for="clinic in topRatedClinics.slice(0, 5)"
            :key="clinic.id"
            :clinic="clinic"
            :distance-km="getClinicDistance(clinic)"
          />
      ...
      <!-- کلینیک‌های ۲۴ ساعته -->
      <section v-if="online24Clinics.length" class="mb-12">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
            <i class="ti ti-clock-24 text-3xl text-emerald-500"></i>
            <span>کلینیک‌های ۲۴ ساعته</span>
          </h2>
          <button
            @click="scrollToAllClinics"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          <ClinicCard
            v-for="clinic in online24Clinics.slice(0, 5)"
            :key="clinic.id"
            :clinic="clinic"
            :distance-km="getClinicDistance(clinic)"
          />
      ...
      <!-- ===== Promotional Banner ===== -->
      <section class="mb-12">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 to-emerald-700 p-5 md:p-6 lg:p-8">
          <!-- Background Pattern -->
      ...
      <!-- همه کلینیک‌ها -->
      <section id="all-clinics-section">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
            ...
            <p class="text-sm text-gray-500 mt-1">
              {{ filteredClinics.length }} کلینیک یافت شد
            </p>
          </div>
        </div>
...
// SEO
useHead({
  title: 'پتومن - کلینیک‌های دامپزشکی',
  meta: [
    { name: 'description', content: 'یافتن بهترین کلینیک‌های دامپزشکی در نزدیکی شما، خدمات کامل، تجهیزات مدرن و خدمات ۲۴ ساعته' },
    { name: 'keywords', content: 'کلینیک دامپزشکی، اورژانس حیوانات، جراحی حیوانات، آزمایشگاه دامپزشکی، رادیولوژی حیوانات' },
  ...
  return tenantStore.tenants
      ...
      .map(t => {
        ...
        return {
          ...
          available24: clinicInfoSetting.is24Hours, // نگاشت isOpen به available24
          phone: t.phone,
          rating: parseFloat(t.rating) || 0,
          reviewsCount: t.reviewsCount || 0,
          //avatar: clinicInfoSetting.logo|| t.documents.logo.thumbnail, // فیلد آواتار در دیتای شما نبود
          avatar: resolveUrl(clinicInfoSetting.logo)|| resolveUrl(t.documents.logo.thumbnail), // فیلد آواتار در دیتای شما نبود
          isOpen: hasAvailableSlots,
          services: t.services, // استخراج نام سرویس‌ها
          hasParking: true, // فیلد پیش‌فرض (چون در دیتا نبود)
          hasEmergency: true, // فیلد پیش‌فرض
          workingHours: clinicInfoSetting.is24Hours? '24 ساعته':'',
          establishedYear: clinicInfoSetting.experience || 0 // سال ساخت از تاریخ ایجاد
        };
      });
});
...
/*const allClinics = [
  {
    ...
    reviewsCount: 127,
    isOpen: true,
    services: ['ویزیت عمومی', 'جراحی', 'آزمایشگاه', 'رادیولوژی'],
    hasParking: true,
    hasEmergency: false,
...

  {
    ...
    reviewsCount: 189,
    isOpen: true,
    services: ['ویزیت تخصصی', 'جراحی', 'بستری', 'اورژانس', 'آزمایشگاه'],
    hasParking: true,
    hasEmergency: true,
    workingHours: '24 ساعته',
    establishedYear: 2010
  },
  {
    id: 3,
    name: 'کلینیک رویال پت',
    ...
    reviewsCount: 156,
    isOpen: true,
    services: ['ویزیت عمومی', 'واکسیناسیون', 'آرایش حیوانات', 'رادیولوژی'],
    hasParking: false,
    hasEmergency: false,
  ...
  },
  {
    ...
    reviewsCount: 98,
    isOpen: true,
    services: ['ویزیت عمومی', 'جراحی', 'دندانپزشکی', 'آزمایشگاه'],
    hasParking: true,
    hasEmergency: false,
...

  {
    ...
    reviewsCount: 134,
    isOpen: false,
    services: ['ویزیت عمومی', 'آزمایشگاه', 'رادیولوژی'],
    hasParking: true,
    hasEmergency: false,
  ...
  },
  {
    ...
    reviewsCount: 203,
    isOpen: true,
    services: ['اورژانس 24 ساعته', 'ویزیت عمومی', 'بستری', 'جراحی'],
    hasParking: false,
    hasEmergency: true,
    workingHours: '24 ساعته',
  ...
  },
  {
    ...
    reviewsCount: 87,
    isOpen: true,
    services: ['ویزیت عمومی', 'واکسیناسیون', 'جراحی کوچک'],
    hasParking: true,
    hasEmergency: false,
  ...
  {
    id: 8,
    name: 'مجتمع درمانی حیوانات ایران',
    ...
    reviewsCount: 245,
    isOpen: true,
    services: ['اورژانس', 'جراحی پیشرفته', 'آی سی یو', 'رادیولوژی', 'آزمایشگاه'],
    hasParking: true,
    hasEmergency: true,
    workingHours: '24 ساعته',
...
const filteredClinics = computed(() => {
  ...
  let results = allClinics.value.filter(clinic => {
    const matchesQuery = !state.q ||
      clinic.name.toLowerCase().includes(state.q.toLowerCase()) ||
      clinic.address.toLowerCase().includes(state.q.toLowerCase()) ||
      clinic.services.some(service => service.toLowerCase().includes(state.q.toLowerCase()))

    const matchesStatus = !state.status ||
      (state.status === 'open' && clinic.isOpen) ||
      (state.status === '24h' && clinic.available24) ||
      (state.status === 'emergency' && clinic.hasEmergency)

    const matchesCity = !state.city || clinic.city === state.city
...
const nearby = computed(() => {
  return filteredClinics.value.filter(clinic => clinic.city === selectedCity.value)
})

const topRatedClinics = computed(() => {
  return [...filteredClinics.value].sort((a, b) => b.rating - a.rating)
})

const online24Clinics = computed(() => {
  return filteredClinics.value.filter(clinic => clinic.available24)
})
...
const pagedAll = computed(() => {
  ...
  return {
    data: filteredClinics.value.slice(startIndex, endIndex),
    totalPages: Math.ceil(filteredClinics.value.length / itemsPerPage),
    total: filteredClinics.value.length,
    page: state.page
  }
})
...
const getClinicDistance = (clinic) => {
...
const scrollToAllClinics = () => {
  const element = document.getElementById('all-clinics-section')
  if (element) {
