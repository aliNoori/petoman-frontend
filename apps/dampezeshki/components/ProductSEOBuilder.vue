...
const allBrands = ref(seoData.brands) // برند‌های دسته‌بندی شده بر اساس حیوان
const allAttributes = ref(seoData.attributes) // ویژگی‌های دسته‌بندی شده بر اساس حیوان
...
const filteredProducts = computed(() => {
  ...
  if (selectedAnimal.value) {
    filtered = filtered.filter(p => 
      p.animals && p.animals.includes(selectedAnimal.value.id)
    )
  }
...
const filteredBrands = computed(() => {
  ...
  if (selectedAnimal.value && allBrands.value[selectedAnimal.value.id]) {
    animalBrands = allBrands.value[selectedAnimal.value.id]
...
const attributes = computed(() => {
  if (selectedAnimal.value && allAttributes.value[selectedAnimal.value.id]) {
    return allAttributes.value[selectedAnimal.value.id]
  }
