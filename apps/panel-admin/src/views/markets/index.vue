  ...
  <div class="space-y-6">
        ...
        <div class="relative">
          <div class="flex items-start justify-between mb-4">
            ...
            <div class="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-xs font-medium">
              ...
              <span class="font-english">+{{ orderStore.periodStats.salesGrowth }}%</span>
            </div>
          </div>
          <div class="space-y-1">
            <p class="text-sm font-medium text-gray-600">کل فروش</p>
            <p class="text-3xl font-bold text-gray-900 font-english" dir="ltr">{{ formatNumber(orderStore.periodStats.totalSales) }}</p>
      ...
      <div
        ...
        <div class="relative">
          ...
          <div class="space-y-1">
            ...
            <p class="text-3xl font-bold text-gray-900 font-english" dir="ltr">{{ formatNumber(orderStore.periodStats.platformFee) }}</p>
            <p class="text-xs text-gray-500">تومان</p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="relative">
          ...
          <div class="space-y-1">
            ...
            <p class="text-3xl font-bold text-gray-900 font-english" dir="ltr">{{ formatNumber(orderStore.periodStats.netIncome) }}</p>
            <p class="text-xs text-gray-500">تومان</p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="relative">
          <div class="flex items-start justify-between mb-4">
            ...
            <div class="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-xs font-medium">
              <i class="ti ti-trending-up text-sm"></i>
              <span class="font-english">+{{ orderStore.periodStats.ordersGrowth }}%</span>
            </div>
          </div>
          <div class="space-y-1">
            <p class="text-sm font-medium text-gray-600">کل سفارشات</p>
            <p class="text-3xl font-bold text-gray-900 font-english" dir="ltr">{{ orderStore.periodStats.totalOrders }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="space-y-4">
        <!-- سفارشات جدید -->
        <div
            class="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-5 hover:shadow-lg transition-all">
        ...
        <!-- پرفروش امروز -->
        <div
            class="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-xl p-5 hover:shadow-lg transition-all">
          <div class="flex items-center justify-between mb-3">
          ...
          <div class="space-y-2">
            <div v-for="(product, index) in topProductsToday.slice(0, 3)" :key="index" class="flex items-center gap-2">
              <div
                  class="w-6 h-6 rounded-full bg-amber-500 text-white text-xs font-bold flex items-center justify-center">
                {{ index + 1 }}
              </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="space-y-2">
          <div v-for="(product, index) in topProducts" :key="index"
            ...
            <div class="text-left flex-shrink-0">
              <p class="text-xs font-bold text-gray-900 font-english" dir="ltr">{{ product.totalSales.toLocaleString() }}</p>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="space-y-2">
          <div v-for="(order, index) in weeklyOrders" :key="index"
               class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
          ...
          <div class="p-6 space-y-6">
            ...
            <div>
              ...
              <div class="space-y-3 bg-gray-50 rounded-lg p-4">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-sm font-bold text-gray-900 font-english"
                        dir="ltr">{{ selectedOrder?.amount.toLocaleString() }} تومان</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-sm text-gray-600">کارمزد پلتفرم (5%)</span>
                  <span class="text-sm font-semibold text-red-600 font-english"
                        dir="ltr">{{ (selectedOrder?.amount * 0.05).toLocaleString() }} تومان</span>
                </div>
                <div class="pt-3 border-t border-gray-200 flex items-center justify-between">
                  <span class="text-sm font-bold text-gray-900">درآمد شما</span>
                  <span class="text-lg font-bold text-green-600 font-english"
                        dir="ltr">{{ (selectedOrder?.amount * 0.95).toLocaleString() }} تومان</span>
                </div>
              </div>
            </div>
            ...
            <div class="flex gap-3 pt-4">
              ...
              <button
                  class="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-colors">
                مشاهده کامل
              </button>
            </div>
          </div>
      ...
      <div class="bg-white lg:rounded-2xl shadow-2xl max-w-md w-full h-full lg:h-auto overflow-y-auto" @click.stop>
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="text-center pb-6 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mb-2">بر اساس <span class="font-bold text-gray-900">{{
                rating.totalReviews
              }}</span> نظر مشتریان</p>

            <!-- نمایش رتبه -->
            <div
                class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-200 mt-2">
            ...
            <div class="flex items-start gap-3 mb-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-700 leading-relaxed mb-3">
                  این امتیازات بر عملکرد فروشگاه شما تاثیر مستقیم دارد. با ارسال به‌موقع سفارشات، رفتار مناسب با مشتریان
                  و ارائه خدمات با کیفیت، امتیاز و رتبه خود را بهبود دهید.
                </p>
          ...
          <button @click="showRatingModal = false"
                  class="w-full px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium transition-colors">
            بستن
          </button>
        </div>
      </div>
...
// تغییر بازه زمانی
// اتصال مستقیم selectedPeriod به استور با قابلیت خواندن و نوشتن
const selectedPeriod = computed({
  get: () => orderStore.selectedPeriod,
  set: (value) => {
    orderStore.selectedPeriod = value
  }
})
...
//orderStore.selectedPeriod = selectedPeriod.value
const timePeriods = [
...
// محاسبه میانگین امتیاز (به صورت عدد)
const averageRating = computed(() => {
  if (!reviews.value || reviews.value.length === 0) return 0
  const totalScore = reviews.value.reduce((sum, review) => sum + (review.rating || 0), 0)
  return totalScore / reviews.value.length
})

// امتیاز فروشگاه
const rating = computed(() => ({
  average: averageRating.value, // عدد است
...
// داده‌های نمودار (دریافت مستقیم از استور بر اساس بازه انتخابی)
const chartDataByPeriod = computed(() => {
  return orderStore.chartData || []
})

// داده‌های آماده برای نمودار
const weeklyData = computed(() => chartDataByPeriod.value)

// داده‌های کارت‌های کناری
const newOrders = computed(()=>orderStore.ordersByStatus.paid)
const shippedOrders = computed(()=>orderStore.ordersByStatus.shipped)
const deliveredOrders = computed(()=>orderStore.ordersByStatus.delivered)
const lowStockItems = ref(9)

const topProductsToday = orderStore.topProductsToday/*ref([
  {name: 'قرص استامینوفن', sales: 24},
  {name: 'شربت سرفه', sales: 18},
  {name: 'ویتامین D3', sales: 15}
])*/

// دسترسی به لیست پرفروش‌ترین‌ها
const topProducts = computed(() => orderStore.topSellingProducts)

// اگر می‌خواهید آیکون‌ها را بر اساس دسته‌بندی تغییر دهید (اختیاری):
const topProductsWithIcons = computed(() => {
  return topProducts.value.map(p => {
    let icon = 'ti ti-package';
    if (p.category.includes('غذا')) icon = 'ti ti-bone';
    else if (p.category.includes('بهداشتی')) icon = 'ti ti-soap';
    else if (p.category.includes('قفس') || p.category.includes('لانه')) icon = 'ti ti-home';
    else if (p.category.includes('لوازم')) icon = 'ti ti-tool';
...
const weeklyOrders = computed(() => orderStore.recentOrders)
